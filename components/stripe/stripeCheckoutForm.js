   ...
   */
	...
	return (
		<div className="form-container">
			{user.token === "" ? (
				<h2 className="title">Login first for pay</h2>
			) : cart.total === 0 ? (
				...
				<form onSubmit={handleFormSubmit} className="form">
					<div className="form__pay">
						...
						<div className="form__card-info">
							...
							/>
							<div className="form__card-try">
                try: 4242424242424242 12/24 999
							</div>
						</div>

						{checkoutError && (
							<div className="form__info-req">{checkoutError}</div>
						)}

						<button className="button-pay" disabled={isProcessing || !stripe}>
							{isProcessing ? "Processing..." : `Pay ${cart.total} $`}
						</button>
						<div className="form__transaction-safe">
							{" "}
