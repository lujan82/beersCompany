	...
	const handleAddToCart = () => {
		...
		if (totalSave !== undefined) {
			setCart({
				...
				items: [
					...cart.items,
					{ id: i.id, totalPacks: totalSave.totalPacks + 1 },
				],
			})
		} else {
			setCart({
	...
	return (
		<div className="shop__beer-item">
			...
			<div className="shop__info-cart">
				<div className="shop__button-cart" onClick={handleRemoveToCart}>
					{" "}
          -{" "}
				</div>
				...
				<div className="shop__button-cart" onClick={handleAddToCart}>
					{" "}
          +{" "}
				</div>
			</div>
